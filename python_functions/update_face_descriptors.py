...
import config
import cv2
from facelib import detection as det
from facelib import recognition as rec


def webcamSaveImages(name, path, detector):
    cap = cv2.VideoCapture(
        0
    )  # "../../../Test_video_gesichterkennung.mp4") # 0 -> Standard Camera Device
    if not cap.isOpened():
    ...
    while cap.isOpened():
        ...
        face_loc = detector.detect(rgb_small_frame)

        process_this_frame = not process_this_frame

        # det.drawFaceLocation(frame, face_loc, name, scaleFactor = 4)
    ...
    print(str(count) + " images were saved from Webcam for user" + name + "\n")
...
print(
    "This tool is used to generate the face encodings for the dlib face recognition algorithm."
)
...
if not os.path.isdir(image_dir):
    print(
        "The training directory doesn't exist. Do you want to specify an alternative directory?    [y/n]"
    )
    answer = input("--> ")
...
print(
    "Do you want to take additional images with your webcam, before generating the encodings?    [y/n]"
)
takeImages = input("--> ")
...
if takeImages == "y" or takeImages == "Y":
    ...
    pathlib.Path(currentUser_dir).mkdir(
        exist_ok=True
...
user_list = [user_dir[1] for user_dir in os.walk(image_dir)][
    0
]  # user_dir[1]: first subfolder layer, [.....][0] of main dir
...
if performTests != "y" and performTests != "Y":
    ...
    cap = cv2.VideoCapture(
        "../../../Test_video_gesichterkennung.mp4"
    )  # 0 -> Standard Camera Device
    while cap.isOpened():
        # Grab a single frame of video
        success, frame = cap.read()
        if not success:
...
print(
    "     Average time for generating encodings: "
    + str(encoding_time)
    + "("
    + str(encoding_counter)
    + " encodings)"
)
print(
    "     Average time for recognition: "
    + str(recognition_time)
    + "("
    + str(recognition_counter)
    + " recognized faces)"
)
print("####################################################################\n")
