...
from facelib import recognition
...
class ConfigHandler:
    ...
    def get_detector():
        detectorType = get("DETECTION_ALGORITHM")
        if detectorType == "haar":
            return FaceDetector_HAAR(
                self.get("HAAR_FACES_LOCATION"), self.get("HAAR_DETECTOR_ARGS")
            )
        elif detectorType == "ocv_dnn":
            return FaceDetector_DNN(
                self.get("DNN_DETECTOR_MODEL_LOCATION"),
                self.get("DNN_DETECTOR_CONFIG_LOCATION"),
            )
        elif detectorTypedetectorType == "dlib_cnn":
            return FaceDetector_DLIB_CNN(
                self.get("DLIB_CNN_MODEL_LOCATION"),
                self.get("DLIB_CNN_DETECTOR_ARGS"),
            )
        else:
    ...
    def get_recognizer():
        ...
        if recognizerModel == "dlib_face_encoding":
            recognizer = FaceRecognizer_DLIB_CNN(
                self.get("DLIB_RECOGNITION_MODEL_LOCATION"),
                self.get("FACE_DESCRIPTOR_LOCATION"),
            )
